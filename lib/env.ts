...
const provider: Provider = parsed.APP_PROVIDER === 'openrouter' || parsed.APP_PROVIDER === 'nanogpt' ? parsed.APP_PROVIDER : 'local';
...
const localUrl = parsed.LOCAL_URL || 'http://host.docker.internal:11434/api/chat';
const systemPrompt = parsed.SYSTEM_PROMPT || '';
const deepSearch = parsed.DEEP_SEARCH ? /^1|true|yes$/i.test(parsed.DEEP_SEARCH) : false;
...
export function withDefaults(input: Partial<NormalizedChatConfig & { provider: Provider | string }>): NormalizedChatConfig {
  const provider = resolveProvider(input.provider as Provider);
  let model = input.model;
  if (!model) {
    model = provider === 'openrouter' ? env.modelOpenrouter : provider === 'nanogpt' ? env.modelNanogpt : env.modelLocal;
  }

  return {
    provider,
    model,
    apiKey: provider === 'openrouter' ? (input.apiKey || env.openrouterApiKey) : provider === 'nanogpt' ? (input.apiKey || env.nanogptApiKey) : undefined,
    localUrl: input.localUrl || env.localUrl,
    nanoBaseUrl: env.nanogptBaseUrl,
    systemPrompt: typeof input.systemPrompt === 'string' ? input.systemPrompt : env.systemPrompt,
  };
}
