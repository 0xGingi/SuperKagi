...
import { env, withDefaults, type NormalizedChatConfig, type Provider } from './env';
...
function sanitizeMessages(messages: IncomingMessage[] | undefined, systemPrompt?: string): ChatCompletionMessageParam[] {
  let sanitized = (messages || [])
    .filter((m) => !m.pending)
    .map((m) => ({ role: m.role, content: normalizeContent(m.content) })) as ChatCompletionMessageParam[];

  if (systemPrompt && systemPrompt.trim().length) {
...
function buildClient(config: NormalizedChatConfig) {
  ...
  const baseURL = isOpenRouter ? 'https://openrouter.ai/api/v1' : isNano ? (config.nanoBaseUrl || env.nanogptBaseUrl) : config.localUrl;
...
export async function runChat(payload: ChatPayload): Promise<string> {
  ...
  try {
    ...
    console.log(
      '[MCP] tools loaded:',
      tools.map((t) => ('function' in t ? t.function.name : (t as any).custom?.name || 'unknown')),
    );
  } catch (err) {
    ...
    for (const toolCall of choice.message.tool_calls) {
      if (toolCall.type === 'function') {
        ...
        try {
          ...
          workingMessages.push({ role: 'tool', content: JSON.stringify(result.content), tool_call_id: toolCall.id } as any);
        } catch (error) {
          console.error('[MCP] error', error);
          workingMessages.push({ role: 'tool', content: `Error: ${(error as Error).message}`, tool_call_id: toolCall.id } as any);
        }
      }
    }
...
export async function streamChat(payload: ChatPayload, onChunk: (text: string) => void) {
  const resolved = withDefaults(payload);
  ...
  while (true) {
    ...
    if (finishReason === 'tool_calls' && toolCalls && toolCalls.length) {
      (messages as any).push({ role: 'assistant', content: '', tool_calls: toolCalls });
      for (const call of toolCalls) {
        if (call?.type === 'function') {
          ...
          try {
            ...
            (messages as any).push({ role: 'tool', content: JSON.stringify(result.content), tool_call_id: call.id });
          } catch (err) {
            console.error('[MCP] error (stream)', err);
            (messages as any).push({ role: 'tool', content: `Error: ${(err as Error).message}`, tool_call_id: call.id });
          }
        }
      }
      continue;
